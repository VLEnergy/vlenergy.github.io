...
d3.csv(csvFilePath_co2).then(data=>{
  ...
  const  co2_data = data.map(d => parseFloat(d[co2]));
  ...
  const chartData =  co2_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-co2')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(co2_data);
  var minValue = d3.min(co2_data);
  var maxValue = d3.max(co2_data);

  var btn = d3.select("#btn-co2");
 ...
 d3.select("#coordinates18").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\nCO2: ${nearestData.y.toFixed(2)}`);
 }
