...
const hrsg="HRSG boiler feedwater inlet temperature";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  hrsg_data = data.map(d => parseFloat(d[hrsg]));
  ...
  const chartData =  hrsg_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-hrsgBoiler')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(hrsg_data);
  var minValue = d3.min(hrsg_data);
  var maxValue = d3.max(hrsg_data);

  var btn = d3.select("#btn-hrsgBoiler");
 ...
 d3.select("#coordinates15").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\n HRSG Boiler Feedwater Inlet Temperature: ${nearestData.y.toFixed(2)}℃`);
 }
