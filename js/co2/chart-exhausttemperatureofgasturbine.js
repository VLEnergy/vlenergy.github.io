...
const exhaustTemp="exhaust temperature of gas turbine";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  exhaustTemp_data = data.map(d => parseFloat(d[exhaustTemp]));
  ...
  const chartData =  exhaustTemp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-exhaustTemp_co2')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(exhaustTemp_data);
  var minValue = d3.min(exhaustTemp_data);
  var maxValue = d3.max(exhaustTemp_data);

  var btn = d3.select("#btn-exhaustTemp_co2");
