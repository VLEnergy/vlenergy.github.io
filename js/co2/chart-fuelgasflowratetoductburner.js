...
const fuelGas="fuel gas flow rate to duct burner";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  fuelGas_data = data.map(d => parseFloat(d[fuelGas]));
  ...
  const chartData =  fuelGas_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-fuelGas_co2')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(fuelGas_data);
  var minValue = d3.min(fuelGas_data);
  var maxValue = d3.max(fuelGas_data);

  var btn = d3.select("#btn-fuelGas_co2");
