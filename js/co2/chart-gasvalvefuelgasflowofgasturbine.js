...
const gasValve="gas valve fuel gas flow of gas turbine";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  gasValve_data = data.map(d => parseFloat(d[gasValve]));
  ...
  const chartData =  gasValve_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-gasValve')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(gasValve_data);
  var minValue = d3.min(gasValve_data);
  var maxValue = d3.max(gasValve_data);

  var btn = d3.select("#btn-gasValve");
