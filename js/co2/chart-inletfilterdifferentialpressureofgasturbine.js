...
const inletFilter="inlet filter differential pressure of gas turbine";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  inletFilter_data = data.map(d => parseFloat(d[inletFilter]));
  ...
  const chartData =  inletFilter_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-inletFilter')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(inletFilter_data);
  var minValue = d3.min(inletFilter_data);
  var maxValue = d3.max(inletFilter_data);

  var btn = d3.select("#btn-inletFilter");
 ...
 d3.select("#coordinates16").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\nInlet filter Differential Pressure of Gas Turbine: ${nearestData.y.toFixed(2)}`);
 }
