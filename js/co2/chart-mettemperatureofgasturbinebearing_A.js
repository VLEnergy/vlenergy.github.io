...
const metA="met temperature of gas turbine bearing_A";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  metA_data = data.map(d => parseFloat(d[metA]));
  ...
  const chartData =  metA_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-co2-metTempA')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(metA_data);
  var minValue = d3.min(metA_data);
  var maxValue = d3.max(metA_data);

  var btn = d3.select("#btn-co2-metTempA");
