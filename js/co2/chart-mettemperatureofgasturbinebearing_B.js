...
const metB="met temperature of gas turbine bearing_B";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  metB_data = data.map(d => parseFloat(d[metB]));
  ...
  const chartData =  metB_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-co2-metTempB')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(metB_data);
  var minValue = d3.min(metB_data);
  var maxValue = d3.max(metB_data);

  var btn = d3.select("#btn-co2-metTempB");
