...
const qualityA="quality of steam leaves radiant chamber_A";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  qualityA_data = data.map(d => parseFloat(d[qualityA]));
  ...
  const chartData =  qualityA_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-qualityA')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(qualityA_data);
  var minValue = d3.min(qualityA_data);
  var maxValue = d3.max(qualityA_data);

  var btn = d3.select("#btn-qualityA");
