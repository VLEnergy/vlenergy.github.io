...
const qualityB="quality of steam leaves radiant chamber_B";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  qualityB_data = data.map(d => parseFloat(d[qualityB]));
  ...
  const chartData =  qualityB_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-qualityB')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(qualityB_data);
  var minValue = d3.min(qualityB_data);
  var maxValue = d3.max(qualityB_data);

  var btn = d3.select("#btn-qualityB");
...
d3.select("#coordinates14").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\n Quality of Steam Leaves Radiant Chamber_B: ${nearestData.y.toFixed(2)}`);
}
