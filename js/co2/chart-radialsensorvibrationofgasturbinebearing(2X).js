...
const radial2x="radial sensor vibration of gas turbine bearing (2X)";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  radial2x_data = data.map(d => parseFloat(d[radial2x]));
  ...
  const chartData =  radial2x_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-radial2x')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(radial2x_data);
  var minValue = d3.min(radial2x_data);
  var maxValue = d3.max(radial2x_data);

  var btn = d3.select("#btn-radial2x");
