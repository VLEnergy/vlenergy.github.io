...
const radial3x="radial sensor vibration of gas turbine bearing (3X)";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  radial3x_data = data.map(d => parseFloat(d[radial3x]));
  ...
  const chartData =  radial3x_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-radial3x')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(radial3x_data);
  var minValue = d3.min(radial3x_data);
  var maxValue = d3.max(radial3x_data);

  var btn = d3.select("#btn-radial3x");
