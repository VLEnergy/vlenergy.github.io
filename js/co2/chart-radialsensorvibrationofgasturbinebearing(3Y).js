...
const radial3y="radial sensor vibration of gas turbine bearing (3Y)";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  radial3y_data = data.map(d => parseFloat(d[radial3y]));
  ...
  const chartData =  radial3y_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-radial3y')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(radial3y_data);
  var minValue = d3.min(radial3y_data);
  var maxValue = d3.max(radial3y_data);

  var btn = d3.select("#btn-radial3y");
