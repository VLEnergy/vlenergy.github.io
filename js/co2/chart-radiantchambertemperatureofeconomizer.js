...
const radiantChamber="radiant chamber temperature of economizer";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  radiantChamber_data = data.map(d => parseFloat(d[radiantChamber]));
  ...
  const chartData =  radiantChamber_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-radiantChamber')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(radiantChamber_data);
  var minValue = d3.min(radiantChamber_data);
  var maxValue = d3.max(radiantChamber_data);

  var btn = d3.select("#btn-radiantChamber");
...
d3.select("#coordinates17").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\n Radiant Chamber Temperature of Economizer: ${nearestData.y.toFixed(2)}℃`);
}
