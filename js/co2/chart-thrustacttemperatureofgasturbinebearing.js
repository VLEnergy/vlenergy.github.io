...
const thrustTemp="thrust act temperature of gas turbine bearing";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  thrustTemp_data = data.map(d => parseFloat(d[thrustTemp]));
  ...
  const chartData =  thrustTemp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-co2-thrustTemp')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(thrustTemp_data);
  var minValue = d3.min(thrustTemp_data);
  var maxValue = d3.max(thrustTemp_data);

  var btn = d3.select("#btn-co2-thrustTemp");
