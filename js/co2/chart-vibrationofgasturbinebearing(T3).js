...
const t3Vibration="vibration of gas turbine bearing  (T3)";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  t3Vibration_data = data.map(d => parseFloat(d[t3Vibration]));
  ...
  const chartData =  t3Vibration_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-co2-t3vibration')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(t3Vibration_data);
  var minValue = d3.min(t3Vibration_data);
  var maxValue = d3.max(t3Vibration_data);

  var btn = d3.select("#btn-co2-t3vibration");
