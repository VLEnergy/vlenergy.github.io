...
const t5Vibration="vibration of gas turbine bearing  (T5)";
var index="Time"


d3.csv(csvFilePath_co2).then(data=>{
    // Extract the column data
  const  t5Vibration_data = data.map(d => parseFloat(d[t5Vibration]));
  ...
  const chartData =  t5Vibration_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-co2-t5vibration')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(t5Vibration_data);
  var minValue = d3.min(t5Vibration_data);
  var maxValue = d3.max(t5Vibration_data);

  var btn = d3.select("#btn-co2-t5vibration");
