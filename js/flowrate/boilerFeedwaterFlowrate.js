const boilerFeedwaterFlowRate="Boiler Feedwater Flow Rate";
var csvFilePath_flowrate="./data/data_flowrate_encrypt.csv";
var index="Time";



d3.csv(csvFilePath_flowrate).then(data=>{
    // Extract the column data
  const boilerFeedwaterFlowRate_data = data.map(d => parseFloat(d[boilerFeedwaterFlowRate]));
  ...
  const chartData =boilerFeedwaterFlowRate_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-flowrate-boilerFeedwaterFlowrate')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(boilerFeedwaterFlowRate_data);
  var minValue = d3.min(boilerFeedwaterFlowRate_data);
  var maxValue = d3.max(boilerFeedwaterFlowRate_data);

  var btn = d3.select("#btn-flowrate-boilerFeedwaterFlowrate");
...
d3.select("#coordinates1").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\n Boiler Feedwater Flow Rate: ${nearestData.y.toFixed(2)} Kg/h`);
}
