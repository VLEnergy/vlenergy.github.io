...
d3.csv(csvFilePath_flowrate).then(data=>{
  ...
  const svg = d3.select('#chart-container-flowrate-chamberTemp')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var btn = d3.select("#btn-flowrate-chamberTemp");
