...
d3.csv(csvFilePath_flowrate).then(data=>{
  ...
  const  flowrate_data = data.map(d => parseFloat(d[flowrate]));
  ...
  const chartData = flowrate_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-flowrate')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(flowrate_data);
  var minValue = d3.min(flowrate_data);
  var maxValue = d3.max(flowrate_data);

  var btn = d3.select("#btn-flowrate");
