const steamToStreamHeaderTemp="Steam to Steam Header Temperature";
var csvFilePath_flowrate="./data/data_flowrate_encrypt.csv";
var index="Time";

d3.csv(csvFilePath_flowrate).then(data=>{
    // Extract the column data
  const  steamToStreamHeaderTemp_data = data.map(d => parseFloat(d[steamToStreamHeaderTemp]));
  ...
  const chartData = steamToStreamHeaderTemp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-flowrate-headerPressure')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(steamToStreamHeaderTemp_data);
  var minValue = d3.min(steamToStreamHeaderTemp_data);
  var maxValue = d3.max(steamToStreamHeaderTemp_data);

  var btn = d3.select("#btn-flowrate-headerPressure");
