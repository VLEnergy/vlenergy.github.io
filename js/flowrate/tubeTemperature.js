...
const boilerTubeTemp="Boiler Tube Temperature";
var index="Time";


d3.csv(csvFilePath_flowrate).then(data=>{
    // Extract the column data
  const  boilerTubeTemp_data = data.map(d => parseFloat(d[boilerTubeTemp]));
  ...
  const chartData = boilerTubeTemp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-flowrate-tubeTemperature')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(boilerTubeTemp_data);
  var minValue = d3.min(boilerTubeTemp_data);
  var maxValue = d3.max(boilerTubeTemp_data);

  var btn = d3.select("#btn-flowrate-tureTemperature");
