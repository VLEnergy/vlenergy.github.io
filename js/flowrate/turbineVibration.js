...
const gasTurbineVibration="Gas Turbine Vibration";
var index="Time";


d3.csv(csvFilePath_flowrate).then(data=>{
    // Extract the column data
  const  gasTurbineVibration_data = data.map(d => parseFloat(d[gasTurbineVibration]));
  ...
  const chartData = gasTurbineVibration_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-flowrate-turbineVibration')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(gasTurbineVibration_data);
  var minValue = d3.min(gasTurbineVibration_data);
  var maxValue = d3.max(gasTurbineVibration_data);

  var btn = d3.select("#btn-flowrate-turbineVibration");
...
d3.select("#coordinates5").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\n Gas Turbine Vibration: ${nearestData.y.toFixed(2)}`);
}
