const steamToStreamHeaderPressure="Steam to Steam Header Pressure";


d3.csv(csvFilePath).then(data=>{
    // Extract the column data
  const  steamToStreamHeaderPressure_data = data.map(d => parseFloat(d[steamToStreamHeaderPressure]));
  ...
  const chartData = steamToStreamHeaderPressure_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-headerPressure')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(steamToStreamHeaderPressure_data);
  var minValue = d3.min(steamToStreamHeaderPressure_data);
  var maxValue = d3.max(steamToStreamHeaderPressure_data);

  var btn = d3.select("#btn-headerPressure");
