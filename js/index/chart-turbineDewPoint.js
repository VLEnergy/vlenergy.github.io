const gasTurbineDewPoint="Gas Turbine Dew Point";


d3.csv(csvFilePath).then(data=>{
    // Extract the column data
  const  gasTurbineDewPoint_data = data.map(d => parseFloat(d[gasTurbineDewPoint]));
  ...
  const chartData = gasTurbineDewPoint_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-gasTurbineDewPoint')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(gasTurbineDewPoint_data);
  var minValue = d3.min(gasTurbineDewPoint_data);
  var maxValue = d3.max(gasTurbineDewPoint_data);

  var btn = d3.select("#btn-gasTurbineDewPoint");
...
d3.select("#coordinates10").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\n Gas Turbine Dew Point: ${nearestData.y.toFixed(2)}`);
}
