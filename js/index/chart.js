const csvFilePath="./data/data_monitor_encrypt.csv";
const ductBurnerTemp="Duct Burner Temperature";
const index="Time"
...
d3.csv(csvFilePath).then(data=>{
  ...
  const ductBurnerTemp_data = data.map(d => parseFloat(d[ductBurnerTemp]));
  ...
  const chartData = ductBurnerTemp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-ductBurnerTemp')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(ductBurnerTemp_data);
  var minValue = d3.min(ductBurnerTemp_data);
  var maxValue = d3.max(ductBurnerTemp_data);

  var btn = d3.select("#btn-ductBurnerTemp");
