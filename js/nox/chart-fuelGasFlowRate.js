...
d3.csv(csvFilePath_nox).then(data=>{
  ...
  const svg = d3.select('#chart-container-nox-fuelGasFlowRate')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var btn = d3.select("#btn-nox-fuelGasFlowRate");
