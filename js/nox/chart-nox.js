...
d3.csv(csvFilePath_nox).then(data=>{
  ...
  const  nox_data = data.map(d => parseFloat(d[nox]));
  ...
  const chartData =  nox_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-nox')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(nox_data);
  var minValue = d3.min(nox_data);
  var maxValue = d3.max(nox_data);

  var btn = d3.select("#btn-nox");
