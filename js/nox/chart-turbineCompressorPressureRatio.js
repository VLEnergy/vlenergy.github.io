var csvFilePath_nox="./data/data_nox_encrypt.csv";
const gasTurbineCompressor="Gas Turbine Compressor Pressure Ratio";
var index="Time";


d3.csv(csvFilePath_nox).then(data=>{
  ...
  const svg = d3.select('#chart-container-nox-turbineCompressorPressureRatio')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var btn = d3.select("#btn-nox-turbineCompressorPressureRatio");
