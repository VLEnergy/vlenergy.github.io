...
const gasTurbineShaftSpeed="Gas Turbine Shaft Speed";
var index="Time";


d3.csv(csvFilePath_nox).then(data=>{
    // Extract the column data
  const  gasTurbineShaftSpeed_data = data.map(d => parseFloat(d[gasTurbineShaftSpeed]));
  ...
  const chartData = gasTurbineShaftSpeed_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-nox-turbineShaftSpeed')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(gasTurbineShaftSpeed_data);
  var minValue = d3.min(gasTurbineShaftSpeed_data);
  var maxValue = d3.max(gasTurbineShaftSpeed_data);

  var btn = d3.select("#btn-nox-turbineShaftSpeed");
