...
const CombustionAir="CombustionAir";
var index_otsg="Time"


d3.csv(csvFilePath_otsg).then(data=>{
    // Extract the column data
  const CombustionAir_data = data.map(d => parseFloat(d[CombustionAir]));
  ...
  const chartData = CombustionAir_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-combustionAir')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(CombustionAir_data);
  var minValue = d3.min(CombustionAir_data);
  var maxValue = d3.max(CombustionAir_data);

  var btn = d3.select("#btn-combustionAir");
