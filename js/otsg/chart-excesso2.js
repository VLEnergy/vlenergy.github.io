...
d3.csv(csvFilePath_otsg).then(data=>{
  ...
  const excessO2_data = data.map(d => parseFloat(d[ExcessO2]));
  ...
  const chartData =excessO2_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-excesso2-otsg')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  const yScale = d3.scaleLinear()
    ...
    timee = d3.timeParse("%m/%d/%Y %H:%M:%S")("1/1/2022  1:00:00");
    console.log(timee);

  var meanValue = d3.mean(excessO2_data);
  var minValue = d3.min(excessO2_data);
  var maxValue = d3.max(excessO2_data);

  var btn = d3.select("#btn-excesso2-otsg");
