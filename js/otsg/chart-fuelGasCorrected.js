var csvFilePath_otsg="./data/OTSG_Powerbi_data.csv";
const FuelGas_Corrected="FuelGas_Corrected";
var index_otsg="Time"


d3.csv(csvFilePath_otsg).then(data=>{
    // Extract the column data
  const FuelGas_Corrected_data = data.map(d => parseFloat(d[FuelGas_Corrected]));

  const timeConv = d3.timeParse('%m/%d/%Y %H:%M');
 
  const time = data.map(d => timeConv(d[index_otsg]));
  console.log(time);


  // Create an array of objects with x and y values
  const chartData = FuelGas_Corrected_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-fuelGasCorrected')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(FuelGas_Corrected_data);
  var minValue = d3.min(FuelGas_Corrected_data);
  var maxValue = d3.max(FuelGas_Corrected_data);

  var btn = d3.select("#btn-fuelGasCorrected");
...
d3.select("#coordinates4").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\n Fuel Gas: ${nearestData.y.toFixed(2)}`);
}
