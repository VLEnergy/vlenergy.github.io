...
const FuelFlow="FuelFlow";
var index_otsg="Time"


d3.csv(csvFilePath_otsg).then(data=>{
    // Extract the column data
  const FuelFlow_data = data.map(d => parseFloat(d[FuelFlow]));
  ...
  const chartData = FuelFlow_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-fuelflow')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(FuelFlow_data);
  var minValue = d3.min(FuelFlow_data);
  var maxValue = d3.max(FuelFlow_data);

  var btn = d3.select("#btn-fuelflow");
