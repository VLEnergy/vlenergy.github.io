...
const CEMSNOx="CEMSNOx";
var index_otsg="Time"


d3.csv(csvFilePath_otsg).then(data=>{
    // Extract the column data
  const CEMSNOx_data = data.map(d => parseFloat(d[CEMSNOx]));
  ...
  const chartData = CEMSNOx_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-cemsnox')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(CEMSNOx_data);
  var minValue = d3.min(CEMSNOx_data);
  var maxValue = d3.max(CEMSNOx_data);

  var btn = d3.select("#btn-cemsnox");
