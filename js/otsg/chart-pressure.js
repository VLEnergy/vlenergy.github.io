...
d3.csv(csvFilePath_otsg).then(data=>{
  ...
  const Pressure_data = data.map(d => parseFloat(d[Pressure]));
  ...
  const chartData = Pressure_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-pressure')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(Pressure_data);
  var minValue = d3.min(Pressure_data);
  var maxValue = d3.max(Pressure_data);

  var btn = d3.select("#btn-pressure");
