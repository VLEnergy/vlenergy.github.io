...
const StackTemp="StackTemp";
var index_otsg="Time"


d3.csv(csvFilePath_otsg).then(data=>{
    // Extract the column data
  const StackTemp_data = data.map(d => parseFloat(d[StackTemp]));
  ...
  const chartData = StackTemp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-stackTemp')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(StackTemp_data);
  var minValue = d3.min(StackTemp_data);
  var maxValue = d3.max(StackTemp_data);

  var btn = d3.select("#btn-stackTemp");
