...
d3.csv(csvFilePath_temp).then(data=>{
  ...
  const  temp_data = data.map(d => parseFloat(d[temp]));
  ...
  const chartData = temp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-temp')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(temp_data);
  var minValue = d3.min(temp_data);
  var maxValue = d3.max(temp_data);

  var btn = d3.select("#btn-temp");
