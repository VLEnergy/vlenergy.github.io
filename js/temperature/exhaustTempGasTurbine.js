const exhaustTempGasTurbine="Exhaust Temperature of  Gas Turbine";
...
d3.csv(csvFilePath_temp).then(data=>{
  ...
  const  exhaustTempGasTurbine_data = data.map(d => parseFloat(d[exhaustTempGasTurbine]));
  ...
  const chartData = exhaustTempGasTurbine_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-temp-exhaustTempGasTurbine')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(exhaustTempGasTurbine_data);
  var minValue = d3.min(exhaustTempGasTurbine_data);
  var maxValue = d3.max(exhaustTempGasTurbine_data);

  var btn = d3.select("#btn-temp-exhaustTempGasTurbine");
...
d3.select("#coordinates3").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\nExhaust Temperature Gas Turbine: ${nearestData.y.toFixed(2)}℃`);
}
