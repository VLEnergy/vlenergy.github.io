const fuelGasFlowRate="Fuel Gas Flow Rate to Duct Burners";
...
d3.csv(csvFilePath_temp).then(data=>{
  ...
  const fuelGasFlowRate_data = data.map(d => parseFloat(d[fuelGasFlowRate]));
  ...
  const chartData =fuelGasFlowRate_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-temp-fuelGasFlowRate')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(fuelGasFlowRate_data);
  var minValue = d3.min(fuelGasFlowRate_data);
  var maxValue = d3.max(fuelGasFlowRate_data);

  var btn = d3.select("#btn-temp-fuelGasFlowRate");
