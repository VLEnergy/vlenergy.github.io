var csvFilePath_temp="./data/data_temp_encrypt.csv";
const gasTurbineCompressor="Gas Turbine Compressor Pressure Ratio";
var index="Time";


d3.csv(csvFilePath_temp).then(data=>{
    // Extract the column data
  const  gasTurbineCompressor_data = data.map(d => parseFloat(d[gasTurbineCompressor]));
  ...
  const chartData = gasTurbineCompressor_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-temp-turbineCompressorPressureRatio')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(gasTurbineCompressor_data);
  var minValue = d3.min(gasTurbineCompressor_data);
  var maxValue = d3.max(gasTurbineCompressor_data);

  var btn = d3.select("#btn-temp-turbineCompressorPressureRatio");
 ...
 d3.select("#coordinates7").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\nGas Turbine Compressor Pressure Ratio: ${nearestData.y.toFixed(2)}`);
 }
