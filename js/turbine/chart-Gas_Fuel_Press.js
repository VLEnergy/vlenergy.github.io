const gasfuelpress="ANALOG.AN_Gas_Fuel_Press";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  gasfuelpress_data = data.map(d => parseFloat(d[gasfuelpress]));
  ...
  const chartData = gasfuelpress_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Gas_Fuel_Press')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(gasfuelpress_data);
  var minValue = d3.min(gasfuelpress_data);
  var maxValue = d3.max(gasfuelpress_data);

  var btn = d3.select("#btn-Gas_Fuel_Press");
...
d3.select("#coordinates9").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\nGas Fuel Supply Pressure: ${nearestData.y.toFixed(2)}KPag`);
}
