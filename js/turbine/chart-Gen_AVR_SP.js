const genavrsp="ANALOG.AN_Gen_AVR_SP";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  genavrsp_data = data.map(d => parseFloat(d[genavrsp]));
  ...
  const chartData = genavrsp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Gen_AVR_SP')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(genavrsp_data);
  var minValue = d3.min(genavrsp_data);
  var maxValue = d3.max(genavrsp_data);

  var btn = d3.select("#btn-Gen_AVR_SP");
  ...
  svg.append('path')
    ...
    svg.append('g')
        ...
        .text("Generator Auto Voltage Regulation Set Point");
