const genexcv="ANALOG.AN_Gen_Exc_V";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  genexcv_data = data.map(d => parseFloat(d[genexcv]));
  ...
  const chartData = genexcv_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Gen_Exc_V')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(genexcv_data);
  var minValue = d3.min(genexcv_data);
  var maxValue = d3.max(genexcv_data);

  var btn = d3.select("#btn-Gen_Exc_V");
  ...
  svg.append('path')
    ...
    svg.append('g')
        ...
        .text("Generator Excitation Voltage");
