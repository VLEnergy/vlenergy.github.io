const genkwsp="ANALOG.AN_Gen_kW_SP";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  genkwsp_data = data.map(d => parseFloat(d[genkwsp]));
  ...
  const chartData = genkwsp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Gen_kW_SP')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(genkwsp_data);
  var minValue = d3.min(genkwsp_data);
  var maxValue = d3.max(genkwsp_data);

  var btn = d3.select("#btn-Gen_kW_SP");
