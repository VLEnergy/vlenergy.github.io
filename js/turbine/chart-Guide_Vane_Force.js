const guidevanforce="ANALOG.AN_Guide_Vane_Force";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  guidevanforce_data = data.map(d => parseFloat(d[guidevanforce]));
  ...
  const chartData = guidevanforce_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Guide_Vane_Force')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(guidevanforce_data);
  var minValue = d3.min(guidevanforce_data);
  var maxValue = d3.max(guidevanforce_data);

  var btn = d3.select("#btn-Guide_Vane_Force");
...
d3.select("#coordinates8").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\nGuide Vane Actuator Force: ${nearestData.y.toFixed(2)}N`);
}
