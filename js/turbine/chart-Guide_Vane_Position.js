const guidevanposition="ANALOG.AN_Guide_Vane_Position";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  guidevanposition_data = data.map(d => parseFloat(d[guidevanposition]));
  ...
  const chartData = guidevanposition_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Guide_Vane_Position')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(guidevanposition_data);
  var minValue = d3.min(guidevanposition_data);
  var maxValue = d3.max(guidevanposition_data);

  var btn = d3.select("#btn-Guide_Vane_Position");
  ...
  svg.append('path')
    ...
    svg.append('g')
        ...
        .text("Guide Vane Actuator Position Feedback");
