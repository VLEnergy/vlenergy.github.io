const lubeoilheaderpress="ANALOG.AN_Lube_Oil_Header_Press";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  lubeoilheaderpress_data = data.map(d => parseFloat(d[lubeoilheaderpress]));
  ...
  const chartData = lubeoilheaderpress_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Lube_Oil_Header_Press')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(lubeoilheaderpress_data);
  var minValue = d3.min(lubeoilheaderpress_data);
  var maxValue = d3.max(lubeoilheaderpress_data);

  var btn = d3.select("#btn-Lube_Oil_Header_Press");
