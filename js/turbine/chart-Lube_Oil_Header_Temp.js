const lubeoilheadertemp="ANALOG.AN_Lube_Oil_Header_Temp";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  lubeoilheadertemp_data = data.map(d => parseFloat(d[lubeoilheadertemp]));
  ...
  const chartData = lubeoilheadertemp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Lube_Oil_Header_Temp')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(lubeoilheadertemp_data);
  var minValue = d3.min(lubeoilheadertemp_data);
  var maxValue = d3.max(lubeoilheadertemp_data);

  var btn = d3.select("#btn-Lube_Oil_Header_Temp");
