const maingasvalveposition="ANALOG.AN_Main_Gas_Valve_Position";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  maingasvalveposition_data = data.map(d => parseFloat(d[maingasvalveposition]));
  ...
  const chartData = maingasvalveposition_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Main_Gas_Valve_Position')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(maingasvalveposition_data);
  var minValue = d3.min(maingasvalveposition_data);
  var maxValue = d3.max(maingasvalveposition_data);

  var btn = d3.select("#btn-Main_Gas_Valve_Position");
  ...
  svg.append('path')
    ...
    svg.append('g')
        ...
        .text("Main Gas Fuel Valve Position Feedback");
...
d3.select("#coordinates13").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\nMain Gas Fuel Valve Position Feedback: ${nearestData.y.toFixed(2)}%`);
}
