...
d3.csv(csvFilePath).then(data=>{
  ...
  const  ngpsp_data = data.map(d => parseFloat(d[ngpsp]));
  ...
  const chartData = ngpsp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-Ngp_SP')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(ngpsp_data);
  var minValue = d3.min(ngpsp_data);
  var maxValue = d3.max(ngpsp_data);

  var btn = d3.select("#btn-Ngp_SP");
  ...
  svg.append('path')
    ...
    svg.append('g')
        ...
        .text("Gas Producer Speed Set Point");
