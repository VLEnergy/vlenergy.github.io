const solonoxt5sp="ANALOG.AN_SoloNox_T5_SP";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  solonoxt5sp_data = data.map(d => parseFloat(d[solonoxt5sp]));
  ...
  const chartData = solonoxt5sp_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-SoloNox_T5_SP')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(solonoxt5sp_data);
  var minValue = d3.min(solonoxt5sp_data);
  var maxValue = d3.max(solonoxt5sp_data);

  var btn = d3.select("#btn-SoloNox_T5_SP");
  ...
  svg.append('path')
    ...
    svg.append('g')
        ...
        .text("SoLoNox Control T5 Set Point");
...
d3.select("#coordinates11").text(`Date and Time: ${nearestData.x.toLocaleDateString()} ${nearestData.x.toLocaleTimeString()} |\nSoLoNox Control T5 Set Point: ${nearestData.y.toFixed(2)}°C`);
}
