const t1temperature="ANALOG.AN_T1_Temperature";
...
d3.csv(csvFilePath).then(data=>{
  ...
  const  t1temperature_data = data.map(d => parseFloat(d[t1temperature]));
  ...
  const chartData = t1temperature_data.map((d, i) => ({ x:timeConv(data[i].Time),y: d }));
  ...
  const svg = d3.select('#chart-container-T1_Temperature')
    .append('svg')
    .attr('width', width)
    .attr('height', height);
  ...
  var meanValue = d3.mean(t1temperature_data);
  var minValue = d3.min(t1temperature_data);
  var maxValue = d3.max(t1temperature_data);

  var btn = d3.select("#btn-T1_Temperature");
